...
async function updateBanner(req: Request, res: Response) {
  try {
    ...
    console.error("Error during updating user banner:", error);
    res.status(500).json({
      error: " Internal server error during updating user banner.",
    });
  }
}

export { getUsers, updateUser, updateAvatar, updateBanner };
